...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (currentPage) {
      case Page.Dashboard:
        return <Dashboard transactions={financialData.transactions} budgets={financialData.budgets} goals={financialData.goals} accounts={financialData.accounts} currentUser={currentUser} />;
      case Page.Transactions:
          return <Transactions transactions={financialData.transactions} addTransaction={addTransaction} addMultipleTransactions={addMultipleTransactions} updateTransaction={updateTransaction} deleteTransaction={deleteTransaction} isMobile={isMobile} />;
      case Page.Accounts:
          return <Accounts accounts={financialData.accounts} addAccount={addAccount} updateAccount={updateAccount} deleteAccount={deleteAccount} />;
      case Page.Budgets:
          return <Budgets budgets={financialData.budgets} addBudget={addBudget} expenses={expenses} updateBudget={updateBudget} deleteBudget={deleteBudget} />;
      case Page.Calendar:
          return <Calendar transactions={financialData.transactions} fds={financialData.fds} />;
      case Page.Investments:
        return <InvestmentTracker sips={financialData.sips} fds={financialData.fds} stocks={financialData.stocks} addSip={addSip} addFd={addFd} addStock={addStock} updateSip={updateSip} deleteSip={deleteSip} updateFd={updateFd} deleteFd={deleteFd} updateStock={updateStock} deleteStock={deleteStock} isMobile={isMobile} />;
      case Page.Debts:
        return <DebtTracker debts={financialData.debts} addDebt={addDebt} updateDebt={updateDebt} deleteDebt={deleteDebt} />;
      case Page.Goals:
        return <GoalsTracker goals={financialData.goals} addGoal={addGoal} updateGoal={updateGoal} deleteGoal={deleteGoal} updateGoalProgress={updateGoalProgress} />;
      case Page.AIAdvisor:
  ...
  const renderAuth = () => {
     switch(authView) {
        ...
        case 'signup':
            return <SignUp onSignUp={handleSignUp} onSwitchToLogin={() => { setAuthView('login'); setAuthError(null); }} error={authError} />;
        case 'forgotPassword':
        ...
        default:
            return <SignUp onSignUp={handleSignUp} onSwitchToLogin={() => setAuthView('login')} error={authError} />;
     }
  }
  ...
  return (
    <div className="flex min-h-screen bg-[var(--bg-primary)]">
      <div className="fixed top-0 left-0 w-full h-full bg-gradient-to-br from-yellow-200 to-amber-500 opacity-10 blur-[150px] -z-10"></div>
