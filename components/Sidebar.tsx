...
import { LayoutDashboard, Wallet, TrendingUp, Landmark, Bot, Target, ArrowRightLeft, University, CircleDollarSign, CalendarDays, LogOut, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
const NavSection: React.FC<{ title: string; isCollapsed: boolean }> = ({ title, isCollapsed }) => {
    if (isCollapsed) return <div className="h-px w-8 bg-[var(--border-subtle)] my-3 mx-auto"></div>;
    return <h3 className="px-4 pt-4 pb-1 text-xs font-bold uppercase text-[var(--text-secondary)] tracking-wider">{title}</h3>;
}
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 overflow-y-auto overflow-x-hidden px-2">
                ...
                <NavSection title="Spendings" isCollapsed={isCollapsed} />
